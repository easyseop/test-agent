...
**브라우저 사망 판별(v3 16차 개정)** — 실행 중 브라우저가 죽으면 제품 실패가 아니라
`infra_error`다. 판별은 두 단계이고, 순서가 중요하다.

1. **오류 문구 매칭** — `Target closed`, `Connection closed while reading from the
   driver` 등 제품 코드로는 만들 수 없는 문구.
2. **생존 확인(`browser.is_connected()`)** — 문구가 애매할 때의 결정적 근거.

문구만으로 부족한 이유는 실측 결과다: Chromium이 죽으면 `Target closed`가 아니라
그냥 `Timeout ...ms exceeded`가 난다(Firefox·WebKit은 `Target closed`). 그렇다고
타임아웃 문구를 실행 불가 목록에 넣으면, 이번엔 **느린 화면·안 뜨는 요소 같은 진짜
제품 버그가 실행 불가로 숨는다**(false success). 그래서 문구를 넓히는 대신 브라우저에
직접 생존을 묻는다 — 엔진 문구에 의존하지 않는 판별이다.

생존 여부를 알 수 없으면(브라우저 핸들 없음·조회 예외) **살아 있는 것으로 본다.**
확신 없이 실행 불가로 올리면 제품 실패가 조용히 묻히기 때문이다.

**접근성 점검의 '확인 불가'(v3 17차)** — axe 주입은 CSP가 인라인 스크립트를 막으면
실패한다(실제 운영 사이트에서 흔하다). 이때 위반 0건으로 보고하면 '접근성 문제 없음'이
되어 조용한 거짓 통과가 된다. 그래서 점검 실패는 별도 상태(`a11y_error`)로 올리고,
**점검 실패가 하나라도 있으면 통과를 주지 않는다**(최소 경고). 규칙이 하나도 실행되지
않은 경우(`rulesRun == 0`)도 같은 취급이다 — 깨끗한 페이지가 아니라 검사가 안 된 페이지다.

비-HTML 문서(CSV·JSON 등)는 점검 대상이 아니며 실패도 위반도 아니다. 엔진마다 다운로드
처리가 달라 WebKit은 `/export.csv`를 화면에 띄우는데, 이때 CSV가 '`<title>` 없음·`lang`
없음'으로 잡히는 거짓 위반이 실측으로 확인됐다.

**브라우저 엔진** — `target.browser`(chromium·firefox·webkit) 또는 CLI `--browser`로
선택하며 CLI가 우선한다. 엔진은 판정 결과를 바꾸는 변수이므로 `meta.browser`에
기록하고 리포트에 표시한다. 시각 기준선은 `baselines/<엔진>/`으로 분리한다 —
엔진마다 글꼴 힌팅·안티에일리어싱이 달라 폴더를 공유하면 전 시나리오가 실패하거나
`--update-baselines` 한 번에 다른 엔진 기준선이 조용히 덮인다.
...
**v3 12차 완료(2026-07-27)**: 시각 기준선 최초 생성 승인 정책 — 기준선이 없어
첫 실행 화면을 자동 채택한 경우 통과가 아니라 **경고(미승인 기준선)** 로 처리.
`--update-baselines`로 사람이 승인해야 통과. 첫 실행의 깨진 화면이 '정상'으로
고정되는 것을 방지. 데모는 기준선 부트스트랩으로 정상 all-pass 유지.

**v3 13차 완료(2026-07-27)**: 병렬 실행(`run --workers N`) — 읽기 전용 시나리오를
워커별 독립 브라우저(스레드 로컬 세션)로 병렬 실행. 쓰기 검증은 직렬·최후 유지,
리포트는 입력 순서로 고정해 결정성 보존, 인증 storage_state는 워커 간 공유. 워커=1은
기존 직렬 동작과 동일. 실측: 데모 19시나리오 워커4에서 판정·순서 동일, 속도 ~2배.

**v3 14차 완료(2026-07-27)**: 접근성 점검 강화 + 선택적 게이팅. 내장 규칙 확장
(제목 레벨 건너뜀·양수 tabindex·main 랜드마크 부재·헤더 없는 데이터 표 추가) 및
`a11y.severity`(info 기본 / warn / fail)로 크롤링 전체의 접근성 위반을 하나의 판정
시나리오로 합쳐 선택적 게이팅. 기본 info는 정보성으로 동작 불변. 데모 alt 없는
이미지·main 부재를 severity=fail에서 검출·exit 1 확인.

**v3 15차 완료(2026-07-27)**: CI 통합·링크 무결성. JUnit XML(`report.xml`) 출력으로
CI가 시나리오별 결과 표시. 깨진 링크 검사(`link_check`) — 크롤링으로 발견한 링크의
HTTP 상태 전수 점검(같은 출처 기본, severity 게이팅). 데모 BUG-6(/help 404) 검출.

**v3 16차 완료(2026-07-27)**: 성능 예산 검사(`perf_checks`) — 페이지 로드 지표
(load/DOMContentLoaded/FCP/responseEnd)를 Performance API로 측정해 예산(ms) 임계로
결정적 게이팅. 타이밍 기반 실패 주입은 flaky하므로 판정 로직은 유닛 테스트로 결정적
검증하고, 데모는 여유 예산으로 측정+통과 시연.

**v3 17차 완료(2026-07-29)**: axe-core 4.12.1(MPL-2.0) 동봉 정밀 접근성 점검.
`a11y.engine: axe`(기본) — WCAG 2.1 A/AA 규칙 100여 개, 한국어 로케일 적용.
CDN이 아니라 저장소 동봉인 이유는 결정성이다(실행 시점마다 규칙이 바뀌면 판정이 흔들린다).
기본 `severity: info`(게이팅 없음) + `min_impact`로 심각한 것부터 승격, `rules_exclude`로
규칙별 제외. **핵심 계약: 주입 실패(CSP 등)는 '위반 0건'이 아니라 '확인 불가'로 판정한다** —
검사를 못 한 페이지를 통과로 세면 조용한 거짓 통과가 된다(§5).

**v3 16차 완료(2026-07-29)**: 멀티 브라우저(`--browser` / `target.browser`).
Chromium 141·Firefox 151·WebKit 26.5 실측 — 정상 모드 3엔진 18/18 통과, 버그 주입
모드 3엔진 모두 동일하게 11통과·8실패(검출 결과가 엔진에 무관함을 실증). 기준선
엔진별 격리, 엔진 기록·표시, 브라우저 사망 판별을 생존 확인 기반으로 개정(§5).

**잔여 백로그**: axe 수준 정밀 a11y(브라우저 내 axe-core 주입 — 현재는 간이 규칙).
— 검증 방법 ①~⑥ 엔진 구현 완료 (⑦은 설계상 참고용).
