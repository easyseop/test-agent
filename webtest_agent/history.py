...
def find_previous_run(output_root: Path, current_run_dir: Path) -> Path | None:
...
def diff_for(output_root: Path, current_run_dir: Path, cur_statuses: dict[str, str]) -> dict | None:
    prev_dir = find_previous_run(output_root, current_run_dir)
    if prev_dir is None:
        return None
