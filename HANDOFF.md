...
auth 11/12 · commerce 8/8 · grid 6/6 · checkout 5/5 · input 4/4 = **34/35
(97.1%)**, 기준선 대비 탐지 회귀·신규 오탐·인프라 오류 **0**.
...
2. **아직 채점되지 않는 능력** — 종류가 달라서 시드 버그로는 못 잰다.
   - 정답원 장애 → `infra_error` 구분: Lab 어댑터가 `infra_error`를 **예외로
     던진다**(`convert.mjs`의 `validateTestAgentOutcome`). '판정 불가가 기대값'인
     프로필을 채점하려면 평가기부터 손봐야 한다. 설계 변경이므로 별건.
   - 설정 지문·대조 행 수·콘솔 에러 회계·트레이스 자격증명: 전부 **리포트의
     성질**이지 앱의 결함이 아니다. 시드 버그가 아니라 리포트 검사로 재야 한다.
3. **미착수 — 정직성 항목 3건** (§4-E에서 갈라져 나온 것)
   - 쓰기 검증의 사후조건 다중화(지금은 단일 수치 delta만 본다)
   - 개인정보 마스킹 — 실제 데이터에 붙을 때만 의미가 있으므로 그때 한다
   - `not_proven`을 리포트 표면까지 노출(모델·종료코드에는 이미 있다)
4. **언어 독립 열 선택** (`column_testids` / `column_indexes`) — 지금은 열을
